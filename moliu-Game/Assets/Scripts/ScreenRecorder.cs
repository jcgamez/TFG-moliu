...
namespace MoliuGame
{
	...
	public class ScreenRecorder : MonoBehaviour
	{
		...
		public int frameRate = 24; // number of frames to capture per second
		...
		void Start()
		{
			...
			gameDirectory = "./Media/" + DateTime.Now.ToString("yyyy_MM_dd--HH_mm_ss");

			print("Capturing to: " + gameDirectory + "/frames/");

			if (!System.IO.Directory.Exists(gameDirectory + "/frames/"))
			{
		...
		private void EncodeAndSave()
		{
			...
			while (threadIsProcessing)
			{
				...
				{
					// Generate file path
					string path = gameDirectory + "/frames/frame" + savingFrameNumber.ToString("D5") + ".bmp";
			...
			VideoCreator.setFFmpegPath("./ffmpeg/");

			string inputDirectory = gameDirectory + "/frames/";
			string outputFile = Path.GetFullPath(gameDirectory + "/video.mp4");
			VideoCreator.AddAllFrames(inputDirectory, outputFile);
